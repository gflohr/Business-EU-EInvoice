...
package Business::EU::EInvoice::CLI;
...
sub commands {
	...
	foreach my $dir (@INC) {
		opendir my $dirh,
			File::Spec->catfile($dir, 'Business', 'EU', 'EInvoice', 'Command')
			or next;
		my @classes =
			map { s/\.pm$//i; "Business::EU::EInvoice::Command::$_" }
...
sub dispatch {
	...
	foreach my $task (@cmds) {
		...
		my $class = 'Business::EU::EInvoice::Command::' . $cmd;
		my $module = class2module $class;
...
', program => $0, years => '2024', version => $Business::EU::EInvoice::VERSION);
...
Business::EU::EInvoice::CLI - Business-EU-EInvoice command-line dispatcher.
